...
void library_foo_init(int *rank, int *size)
{
    ...
    if (!getenv("PMIX_NAMESPACE")) {
        /* Skip group modification if we are not running with PMIx.
         * PMIx is required for partial PM fence which is currently
         * only supported for PMIx but not for PMI1/2. */
        goto mod_group_end;
    }
