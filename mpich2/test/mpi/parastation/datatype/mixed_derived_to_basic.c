...
int main(int argc, char **argv)
{
    ...
    rbuffer = malloc(factor * count * blocklength * basic_size * nprocs);
    ...
    test_handle(MPI_Allgather
                (sbuffer, factor, type_derived, rbuffer, factor * count * blocklength,
                 type_basic, MPI_COMM_WORLD));

    for (int i = 0; i < nprocs; i++) {
        for (int j = 0; j < factor * count * blocklength; j++) {
            int pos = j + factor * count * blocklength * i;
    ...
    for (int i = 0; i < nprocs; i++) {
        rcounts[i] = factor * count * blocklength;
        rdispls[i] = (i * factor * count * blocklength) + i;
    }
    ...
    test_handle(MPI_Allgatherv
                (sbuffer, factor, type_derived, rbuffer, rcounts, rdispls, type_basic,
                 MPI_COMM_WORLD));

    for (int i = 0; i < nprocs; i++) {
        for (int j = 0; j < factor * count * blocklength; j++) {
            int pos = (j + factor * count * blocklength * i) + i;
            test_assert(rbuffer[pos] == 1.0 * j + (j / blocklength) * (stride - blocklength) + i,
                        "mismatch at position %d", pos);
        }
    }
    ...
    if (rank == root) {
        ...
        test_handle(MPI_Bcast
                    (rbuffer, factor * count * blocklength, type_basic, root, MPI_COMM_WORLD));

        for (int i = 0; i < factor * count * blocklength; i++) {
            test_assert(rbuffer[i] == 1.0 * i + (i / blocklength) * (stride - blocklength),
    ...
    if (rank == root) {
        ...
        test_handle(MPI_Scatter
                    (NULL, 0, MPI_DATATYPE_NULL, rbuffer, factor * count * blocklength, type_basic,
                     root, MPI_COMM_WORLD));

        for (int i = 0; i < factor * count * blocklength; i++) {
            test_assert(rbuffer[i] == 1.0 * i + (i / blocklength) * (stride - blocklength) + rank,
                        "mismatch at position %d", i);
        }
    }
    ...
    if (rank == root) {
        ...
        test_handle(MPI_Scatterv
                    (NULL, NULL, NULL, MPI_DATATYPE_NULL, rbuffer, factor * count * blocklength,
                     type_basic, root, MPI_COMM_WORLD));
