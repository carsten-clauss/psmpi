...
int main(int argc, char **argv)
{
    ...
    /* Init global vars */
    int align = 0;
#if defined(SCATTER) || defined(SCATTERV) || defined(ALLTOALL) || defined(ALLTOALLV)
    align = 1;
#endif
    init_datatype(align, nprocs);

#if defined(ALLGATHER)
    sbuffer = malloc((factor * count * blocklength + 1) * basic_size);
    rbuffer = malloc(factor * count * stride * basic_size * nprocs);

    for (int i = 0; i < factor * count * blocklength + 1; i++) {
        sbuffer[i] = (type_basic_t) i + rank;
    }

    test_handle(MPI_Allgather
                (sbuffer, 1, type_contig, rbuffer, factor, type_derived, MPI_COMM_WORLD));

    for (int p = 0; p < nprocs; p++) {
        for (int i = 0, j = 0, k = 1; i < factor * count * stride; i++, k++) {
    ...
    test_handle(MPI_Alltoallv
                (sbuffer, scounts, sdispls, type_contig, rbuffer, rcounts, rdispls,
                 type_derived, MPI_COMM_WORLD));
    ...
    if (rank == root) {
        ...
        MPI_Bcast(sbuffer, 1, type_contig, root, MPI_COMM_WORLD);
    ...
    if (rank == root) {
        ...
        test_handle(MPI_Gather
                    (sbuffer, 1, type_contig, rbuffer, factor, type_derived, root, MPI_COMM_WORLD));
        ...
        MPI_Gather(sbuffer, 1, type_contig, NULL, 0, MPI_DATATYPE_NULL, root, MPI_COMM_WORLD);
    }
    ...
    if (rank == root) {
        ...
        test_handle(MPI_Gatherv
                    (sbuffer, 1, type_contig, rbuffer, rcounts, rdispls, type_derived,
                     root, MPI_COMM_WORLD));
        ...
        MPI_Gatherv(sbuffer, 1, type_contig, NULL, NULL, NULL, MPI_DATATYPE_NULL, root,
                    MPI_COMM_WORLD);
    }
    ...
    if (rank == root) {
        ...
        MPI_Scatter(sbuffer, 1, type_contig, rbuffer, factor, type_derived, root, MPI_COMM_WORLD);
    ...
    if (rank == root) {
        ...
        MPI_Scatterv(sbuffer, scounts, sdispls, type_contig, rbuffer, factor,
