                            ...
                            MPI_Datatype dtype, MPI_Op op, int root, MPIR_Comm * comm_ptr)
{
    ...
    MPIDI_COMMON_UCC_WRAPPER_ENTER(reduce);

    MPIDI_COMMON_UCC_WRAPPER_EXECUTE(reduce, sbuf, rbuf, count, dtype, op, root, comm_ptr, &req);

    MPIDI_COMMON_UCC_WRAPPER_EXIT(reduce);
}
