...
int MPID_Intercomm_exchange_map(MPIR_Comm * local_comm_ptr, int local_leader,
                                ...
                                int *remote_size, uint64_t ** remote_lpids, int *is_low_group)
{
    int mpi_errno = MPI_SUCCESS;
    int singlePG;
    int local_size = 0;
                              ...
                              int vcrt_offset)
{
    ...
    if (mapper->type == MPIR_COMM_MAP_TYPE__DUP) {
        ...
        for (i = 0; i < mapper->src_mapping_size; i++)
            (*dest_vcrt)->vcr[i + vcrt_offset] =
                MPIDI_VC_Dup(src_vcrt->vcr[mapper->src_mapping[i]]);
    }
}
        ...
        switch (mapper->dir) {
            ...
            case MPIR_COMM_MAP_DIR__L2L:
                if (src_comm->comm_kind == MPIR_COMM_KIND__INTRACOMM &&
                    ...
                    MPID_PSP_mapper_dup_vcrt(src_comm->vcrt, &comm->vcrt, mapper,
                    ...
                    MPID_PSP_mapper_dup_vcrt(src_comm->vcrt, &comm->local_vcrt, mapper,
                                             mapper->src_comm->local_size, vcrt_size, vcrt_offset);
                    MPID_PSP_comm_set_local_vcrt(comm, comm->local_vcrt);
                } else if (src_comm->comm_kind == MPIR_COMM_KIND__INTERCOMM &&
                           comm->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
                    MPID_PSP_mapper_dup_vcrt(src_comm->local_vcrt, &comm->vcrt, mapper,
                                             mapper->src_comm->local_size, vcrt_size, vcrt_offset);
                    MPID_PSP_comm_set_vcrt(comm, comm->vcrt);
                } else {
                    MPID_PSP_mapper_dup_vcrt(src_comm->local_vcrt, &comm->local_vcrt, mapper,
                                             mapper->src_comm->local_size, vcrt_size, vcrt_offset);
                    MPID_PSP_comm_set_local_vcrt(comm, comm->local_vcrt);
                }
